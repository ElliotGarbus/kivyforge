...
filesystem is only probed when collisions exist, so runtimes without any are
untouched: the Windows runtime was scanned (0 collisions), and the macOS one
has passed through the check in CI without tripping it (§7, 2026-09-24 —
inferred, not scanned). **Terminfo was deliberately not
...
| 2026-09-24 | Windows `amd64` (`windows-gate`, release) | §5.6 path length, **CI with long paths off** (`windows_onedir`, PR #16) | GitHub `windows-latest`; `LongPathsEnabled` switched from 1 to 0 in the job | **First reproduction with long paths off; exact to the character.** Precondition passed (registry 0, and a file made through `\\?\` unreachable from a fresh process). The release bundle's deepest entry is **104** characters (`python\Lib\site-packages\pywin32-312.dist-info\licenses\com\win32comext\mapi\src\MAPIStubLibrary\LICENSE`), not the dev build's 105: stripping moved pip's `__pycache__` `.pyc` to the legacy location. Predicted headroom **154**. At a 154-character folder: deepest file opens (rc 0) and `pip._internal.resolution.resolvelib.found_candidates` imports (rc 0). At 155: `FileNotFoundError: [Errno 2] No such file or directory` on the deepest file. The bisection found the deepest *import* survives up to a **177**-character folder: its module file is 81 characters, 259 in total, exactly `MAX_PATH`. So a user hits the import limit (177) later than the file limit (154), which is set by a license file nothing imports. 2 passed. Note: the runner reported `LongPathsEnabled=1` before the step, confirming CI had never seen the default setting before. |
| 2026-09-24 | iOS device + simulator `arm64` (`hello-kivy`), macOS `arm64` (`dice-roller`) | provisioning-profile fix + end-user docs walk (local) | macOS 26.6.2, Xcode 26.6; iPhone14,3 paired but **unavailable** (device builds only, no install) | Per [`mac-docs-and-provisioning-prompt.md`](mac-docs-and-provisioning-prompt.md). **The change set behaves as specified:** `provisioning_profile` by name, UUID, `.mobileprovision` path (written to the pbxproj as the file's **UUID**), unknown name (`FAIL`), and not-installed path (`WARN` + double-click hint) all match the expected `doctor` results and specifiers against a real installed profile; the entitlements pre-flight `FAIL`s a pinned name under manual signing and stops `build --device` before `xcodebuild` (0 invocations), `WARN`s under auto; `--arch aarch64` is refused on iOS; `uv tool install` and `pipx install` both ship pip 26.2.1 and `lock -p macos` `ok: true`. **Five defects, none introduced by the change set:** (1) `run -p ios` never streams the app console — `run_command` spawns `simctl launch --console-pty` with `capture_output=True`, true since 2026-07-10, confirmed under a real pty against a direct `simctl` call that streams; (2) `auto_signing = true` + any pinned profile fails `xcodebuild` ("conflicting provisioning settings") while `doctor` passes — the specifier is emitted regardless of `CODE_SIGN_STYLE`; (3) an Xcode-managed profile under `auto_signing = false` fails `xcodebuild` ("is Xcode managed") while `doctor` passes — kivyforge never reads `IsXcodeManaged`, so with (2) no setting builds with the only kind of profile automatic signing installs; (4) the new iOS `--arch` refusal recommends `x86_64`, which ends in an unclassified `WheelSelectionError` traceback; (5) the built `.app` is mode `0700` (`mkdtemp` swap in `macos/bundle.py`), ad-hoc and notarized alike. **Docs:** the iOS and desktop quickstarts, `ios/run.md`, `macos/signing.md` (Developer ID sign + notarize **Accepted** + staple, then `codesign`/`spctl`/`stapler` all pass) and `macos/dmg.md` (DMG mounts with app + Applications link, signed, notarized **Accepted**, `spctl -t open` accepted) are correct as written apart from the console-streaming claim and the provisioning guidance (defects 1–3). `ios/signing.md`'s `.ipa` path confirmed via `--export-method development`. Not proven: a manually signed device build (no manual profile), any install (device unavailable). `pylock.ios.toml` untouched. Full detail: [`mac-docs-and-provisioning-findings.md`](mac-docs-and-provisioning-findings.md). |
