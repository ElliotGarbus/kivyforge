...
`revendor_*` jobs stay on `3.x`. None is the sole proof of a *platform* —
they are cross-cutting checks, or they exercise a C toolchain and a signing
tool whose behaviour does not turn on the Python minor. §5.8 asked for the
platform proofs to be pinned, not for everything to be, and pinning the rest
would give up the early warning that floating buys.
