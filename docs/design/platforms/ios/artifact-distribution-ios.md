...
The kivy-ios 2.x recipe model needed an explicit per-framework list (`recipe.pbx_frameworks`) only because it linked **static** libraries into the app binary, forcing the app target to resolve every transitive system symbol. The all-dynamic model removes that need entirely — there is no `ios-system-frameworks.toml` manifest, no `system_frameworks` key, and no scan step. See [iOS pyproject §"System frameworks are not declared"](pyproject-ios.md#system-frameworks-are-not-declared--they-link-transitively) and [Xcode project generation](xcode-project-generation.md).
