...
6. Runs the entry module as `__main__` via `runpy.run_module(ENTRY_POINT, run_name="__main__", alter_sys=True)`, matching every other platform since 2026-09-21 (was `PyImport_ImportModule(ENTRY_POINT)` before) — see the [common spec's entry_point callout](../../common/01-pyproject-kivy-spec.md) for the unified contract.
...
- Add the **bootstrap baseline** — the frameworks the generated `main.m` shell link-references regardless of dependencies (Foundation, UIKit) — to Link Binary With Libraries. This is the *only* static framework list the toolchain wires. Frameworks needed by *libraries* (Metal, AVFoundation, CoreGraphics, … for Kivy) are **never** declared here: every dependency ships as a **dynamic** framework that records its own SDK dependencies as `LC_LOAD_DYLIB` load commands, and `dyld` resolves them transitively at launch (app → `kivy…framework` → `Metal`/`AVFoundation`/…). See [iOS pyproject §"System frameworks are not declared"](01-pyproject-ios.md#system-frameworks-are-not-declared--they-link-transitively).
...
The pbxproj generator sets the following (from the python.org docs §7.2.2 step 6) on every app target build configuration. Users cannot override these via `[tool.kivy.ios.xcode.build_settings]` (they're on the reserved list — see [iOS pyproject §`[tool.kivy.ios.xcode.build_settings]`](01-pyproject-ios.md#toolkivyiosxcodebuild_settings) for the full reserved set):
...
| `GCC_PREPROCESSOR_DEFINITIONS` | `$(inherited) KIVYFORGE_REQUIRES_SDL=1` *(only when `SDL3.xcframework` is staged — the same signal `HEADER_SEARCH_PATHS` uses to decide whether to add SDL3's headers)* | Makes `kivyforge_bootstrap.m`'s `#if defined(KIVYFORGE_REQUIRES_SDL) && !__has_include(<SDL3/SDL_main.h>)` guard fail the build with an actionable error if a Kivy app's SDL3 headers go missing (a broken vendoring step, a manually edited `HEADER_SEARCH_PATHS`), instead of silently compiling the headless fallback path — an app that launches but can never open a window. `$(inherited)` preserves Xcode's own per-configuration definitions (e.g. `DEBUG=1`). Credit: [PR #1](https://github.com/ElliotGarbus/kivyforge/pull/1) (kengoon). |
