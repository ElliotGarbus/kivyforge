...
6. Imports the entry module via `PyImport_ImportModule(ENTRY_POINT)`. Same as Android; *not* the same as Linux/macOS/Windows, which run it as `__main__` — see the [common spec's entry_point callout](../../common/01-pyproject-kivy-spec.md) for the full split and the portable style (`App().run()` unconditionally, no `__main__` guard).
