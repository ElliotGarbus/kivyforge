...
7. *(Only when `--simulator`, `--device`, or `--release` is passed)* Invoke `xcodebuild` for the selected configuration.
   ...
   - **Entitlements pre-flight.** Also `--device`/`--release` only. `[tool.kivy.ios.entitlements]` is written verbatim into `<app>.entitlements`, but `codesign` requires the app's entitlements to be a **subset** of what the provisioning profile grants. A mismatch otherwise surfaces as `Provisioning profile "…" doesn't include the <key> entitlement` deep inside `xcodebuild`, naming neither the pyproject key that caused it nor the App ID capability that fixes it. When `[tool.kivy.ios.entitlements]` is non-empty and `[tool.kivy.ios.signing].provisioning_profile` is set, `build` finds the profile file (an installed profile matched by name or UUID, or a `.mobileprovision` path — see the doctor table's Provisioning profile row), reads its embedded plist (a `.mobileprovision` is a CMS envelope wrapping the plist verbatim, so it is sliced out of the bytes rather than shelled out to `security cms` — keeping the check pure and testable off a macOS host), takes its `Entitlements` dictionary, and compares:

     ```
     Error: entitlements declared in pyproject.toml are not granted by the provisioning profile.
       Profile: ios_distribution.mobileprovision  (App ID: org.example.myapp)
       Not granted:
     ...
     A mismatch is a non-zero exit before `xcodebuild` runs: the pinned profile is the one that will sign, so a missing entitlement is a certain signing failure.

     The same pre-flight refuses the two pins Xcode itself refuses, each of which otherwise fails only at the end of the build:
     - a `provisioning_profile` with `auto_signing = true` — Xcode rejects `PROVISIONING_PROFILE_SPECIFIER` under `CODE_SIGN_STYLE = Automatic` ("conflicting provisioning settings");
     - an Xcode-managed profile (`IsXcodeManaged = true`, e.g. `iOS Team Provisioning Profile: ...`) under manual signing — Xcode requires a manually managed profile.

     Until 2026-09 an ungranted entitlement under `auto_signing = true` was a WARN (`KF-ENTITLEMENTS-UNGRANTED`) on the theory that `-allowProvisioningUpdates` could register it mid-build. Xcode's refusal of any pin under automatic signing made that path unreachable ([Mac findings](../../dev/mac-docs-and-provisioning-findings.md) defect 2); the code is retired.

     Skipped entirely when no profile is pinned (nothing to compare against), and for `--simulator` and the default no-flag `build` — neither signs. `run --device` and `package` run the same pre-flight. Entitlement *values* are never validated, only key presence; Apple's grant semantics vary per key and kivyforge does not model them.
...
| Provisioning profile | project | If `[tool.kivy.ios.signing].provisioning_profile` is set, it resolves. A name or UUID (Xcode's `PROVISIONING_PROFILE_SPECIFIER` contract) must match an installed profile in `~/Library/Developer/Xcode/UserData/Provisioning Profiles` or `~/Library/MobileDevice/Provisioning Profiles` — UUID match first, then `Name`, newest file among duplicate names — else FAIL. A value ending in `.mobileprovision` is a path (absolute or project-relative): FAIL if missing or unreadable, WARN if that profile's UUID is not installed (Xcode signs only with installed profiles; `build` passes the file's UUID as the specifier). Also FAIL, before any lookup, when `auto_signing = true` (Xcode refuses a pin under automatic signing), and FAIL when the resolved profile is Xcode-managed (`IsXcodeManaged`), which manual signing refuses. |
| Entitlements vs. profile | project | If `[tool.kivy.ios.entitlements]` is non-empty **and** a `provisioning_profile` is pinned under `auto_signing = false`, read the profile's embedded plist and compare declared keys against its `Entitlements` dictionary. FAIL on a missing key (the pinned profile is the one that will sign, so it is a certain `codesign` failure); the hint names each key the profile does not grant, the App ID, and the two fixes — enable the capability at developer.apple.com and regenerate the profile, or drop the key. SKIP if either side is unset, or under `auto_signing = true`, where the Provisioning profile check already FAILs the pin. Mirrors the [`build` entitlements pre-flight](#kivyforge-build). |
...
| App-local native binaries | project | Scan `app_dir` for `.so`/`.dylib` files; FAIL on any non-iOS-architecture binary (a macOS-compiled extension dropped in `app/` won't load on device — native code belongs in an iOS wheel, see [iOS artifact distribution §"App-specific native extensions"](03-artifact-distribution-ios.md)). WARN on any binary whose platform can't be read (truncated, corrupt, or not a Mach-O) — it can't be confirmed iOS-safe. |
...
See [recipe triage §"Deleted recipes — `ios` recipe"](07-recipe-triage.md).
