...
7. *(Only when `--simulator`, `--device`, or `--release` is passed)* Invoke `xcodebuild` for the selected configuration.
   ...
   - **Entitlements pre-flight.** Also `--device`/`--release` only. `[tool.kivy.ios.entitlements]` is written verbatim into `<app>.entitlements`, but `codesign` requires the app's entitlements to be a **subset** of what the provisioning profile grants. A mismatch otherwise surfaces as `Provisioning profile "…" doesn't include the <key> entitlement` deep inside `xcodebuild`, naming neither the pyproject key that caused it nor the App ID capability that fixes it. When `[tool.kivy.ios.entitlements]` is non-empty and `[tool.kivy.ios.signing].provisioning_profile` is set, `build` finds the profile file (an installed profile matched by name or UUID, or a `.mobileprovision` path — see the doctor table's Provisioning profile row), reads its embedded plist (a `.mobileprovision` is a CMS envelope wrapping the plist verbatim, so it is sliced out of the bytes rather than shelled out to `security cms` — keeping the check pure and testable off a macOS host), takes its `Entitlements` dictionary, and compares:
     ...
     Severity follows the signing mode, because that determines whether the mismatch is actually fatal:
     - `auto_signing = false` — **FAIL**, non-zero exit before `xcodebuild` runs. The pinned profile is the one that will be used, so a missing entitlement is a certain signing failure.
     - `auto_signing = true` — **WARN**, and the build proceeds. `-allowProvisioningUpdates` may register the capability and regenerate the profile mid-build, so kivyforge names the keys that will require App ID capabilities rather than blocking a build that Xcode can still resolve.

     Skipped entirely when no profile is pinned (nothing to compare against), and for `--simulator` and the default no-flag `build` — neither signs. Entitlement *values* are never validated, only key presence; Apple's grant semantics vary per key and kivyforge does not model them.
...
| Entitlements vs. profile | project | If `[tool.kivy.ios.entitlements]` is non-empty **and** a `provisioning_profile` is pinned, read the profile's embedded plist and compare declared keys against its `Entitlements` dictionary. FAIL when `auto_signing = false` (the pinned profile is the one that will sign, so a missing key is a certain `codesign` failure); WARN when `auto_signing = true` (`-allowProvisioningUpdates` may register the capability at build time). Either way the hint names each key the profile does not grant, the App ID, and the two fixes — enable the capability at developer.apple.com and regenerate the profile, or drop the key. SKIP if either side is unset. Mirrors the [`build` entitlements pre-flight](#kivyforge-build). |
