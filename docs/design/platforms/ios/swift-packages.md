...
This document adds a third way for an app to declare a native dependency: a
**Swift Package Manager (SPM) package**, declared by Git URL (or local path) and
resolved by Xcode. It fills a gap in the other two native channels — a growing
...
> resurrect the kivy-ios 2.x recipe system (building Python, OpenSSL, libffi, and
> ~50 libraries through a bespoke build framework on the user's mac), and it
> does not cross-compile Python C extensions. Those are the genuinely painful,
> non-reproducible, toolchain-version-sensitive steps the architecture
> exists to eliminate.
...
- It is driven by **Xcode's own, first-class package manager** — `xcodebuild`
  resolves, fetches, compiles, and embeds the package end-to-end. kivyforge writes
...
principle that matters — *kivyforge owns no from-source build pipeline* — is
preserved.
...
kivyforge does **not** download, hash, extract, or dedupe SPM artifacts for this
channel — that machinery (used by `[[tool.kivyforge.xcframeworks]]`) does not
...
The pylock principle "every artifact pinned by URL + content hash" cannot extend
...
- For **binary** targets, SPM additionally records the `.binaryTarget` checksum;
  Xcode verifies it on fetch. kivyforge carries that checksum through for audit.
...
`.xcframework` archives remain content-hash-pinned and verified by kivyforge as
...
`Package.resolved`, not from a kivyforge-computed output hash (see
...
2. Record one `[[tool.kivyforge.swift_packages]]` entry: the declared `url`/`path`
...
2. For each product, ensure an `XCSwiftPackageProductDependency` on the app
   ...
   `add_package_dependency` adds the link build file automatically; kivyforge adds
...
> a plain JSON file kivyforge writes directly (pbxproj does not manage it).
...
separate concern with a clean boundary: **kivyforge's responsibility ends once the
...
and unreachable by pyobjus, regardless of any project wiring kivyforge does.
...
`@objc` surface, kivyforge cannot expose it — that is not a wiring gap but an
...
`swift_packages` (specified above). kivyforge wires the local shim like any other
...
and embeds it **transitively** whether or not kivyforge knows about it. There are
...
package — there is no reliable static, pre-build check. kivyforge documents the
...
- **Privacy manifests**: SPM-embedded `.xcframework`s land in the built app's
  ...
  because kivyforge does not stage them, the `doctor` `Frameworks/` scan may not
...
Because kivyforge does not compute an output hash for SPM products, the integrity
guarantees for this channel are SPM's, not kivyforge's: the pinned Git `revision`
fixes the source, and binary targets carry SPM's `.binaryTarget` checksum (Xcode
verifies it on fetch). This is weaker than the kivyforge-verified `sha256` applied
to wheels and direct `.xcframework` archives, and is the documented cost of
supporting source SPM. Users who require kivyforge-side hash verification of a
...
1. **Embedding nuance — resolved (Phase 0 spike, see
   [swift-spm-findings](../../dev/swift-spm-findings.md)).** For a
   dynamic SPM product, kivyforge must add an **explicit** Embed Frameworks /
   ...
   **no** empty `.swift` stub to consume a Swift package at the iOS 13 floor (the
   ...
   which kivyforge already needs for its other embedded frameworks.
2. **`Package.resolved` format version — resolved.** kivyforge writes schema
   **version 2** (`{ "pins": [ … ], "version": 2 }`, the flattened layout with
   ...
   forward-compatible. Implemented in `kivy_ios/project/swift_packages.py`
   (`PACKAGE_RESOLVED_VERSION = 2`).
3. **Schema version — resolved.** Adding `swift_packages` is additive to
   `[tool.kivy.ios]` (new optional subtable) and to `[tool.kivyforge]` in the
   lock, so by the iOS overlay evolution policy it does **not** bump
   `[tool.kivy.ios].schema_version` nor the lock's
   `[tool.kivyforge].schema_version`. Both remain at `1`.
