...
`app/proguard-rules.pro` is generated by concatenating, in order (only the
baseline is implemented today; the `proguard` table is specified but rejected
by the loader, see
[pyproject-android](01-pyproject-android.md#toolkivyandroidproguard--r8-keep-rules)):
