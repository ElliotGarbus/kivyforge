...
| R8 keep rules | project | **Not implemented**, with the `proguard` table it inspects ([pyproject-android](01-pyproject-android.md#toolkivyandroidproguard--r8-keep-rules)). When `[tool.kivy.android.build_settings].minify` is on: **WARN** if the app ships `[tool.kivy.android.src].java`/`.kotlin` but declares no `[tool.kivy.android.proguard]` rules — R8 removes or renames anything reached only through `autoclass`, and the failure surfaces as a runtime `ClassNotFoundException` on a release build rather than at build time. Also **FAIL** on a `rules_files` entry that does not exist or escapes the project. SKIP when `minify` is off (the rules are unused) or when the app ships no Java. Advisory by nature: kivyforge cannot know which classes Python reaches, only that the risk is unmitigated. |
