"""The smallest Kivy app that still proves the Windows build path works.

Deliberately trivial. This is a build fixture, not a demonstration — every
line here is one the CI job has to stage, byte-compile and strip, so there is
no reason for it to be longer than it is.
...
class WindowsGateApp(App):
    def build(self):
        return Label(text="windows-gate")
