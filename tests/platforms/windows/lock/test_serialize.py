"""pylock.windows.toml round-trip + drift + malformed handling."""
...
from kivyforge.platforms.windows.lock import build_windows_lockfile, dumps, loads
...
def _build(text, tmp_path, **kw):
    ...
    return build_windows_lockfile(
        cfg,
        text,
        project_root=tmp_path,
        resolver=FakeWindowsResolver(**kw),
        runtime_provider=FakeRuntimeProvider(),
    )


class TestRoundTrip:
    def test_round_trips(self, windows_pyproject, tmp_path):
        lock = _build(windows_pyproject, tmp_path)
        again = loads(dumps(lock))
        assert again.platform == "windows"
        ...
        assert {a.arch for a in again.python_runtime.artifacts} == {"amd64"}

    def test_deterministic(self, windows_pyproject, tmp_path):
        lock = _build(windows_pyproject, tmp_path)
        assert dumps(lock) == dumps(loads(dumps(lock)))

    def test_has_expected_tables(self, windows_pyproject, tmp_path):
        text = dumps(_build(windows_pyproject, tmp_path))
...
class TestNativeBinaries:
    _NB_TABLE = (
        ...
        'sdk = { version = "1.0", source = "binaries/windows/sdk.dll" }\n'
    )

    def test_vendored_binary_is_pinned(self, windows_pyproject, tmp_path):
        binaries = tmp_path / "binaries" / "windows"
        binaries.mkdir(parents=True)
        (binaries / "sdk.dll").write_bytes(b"MZ sdk payload")
        lock = _build(windows_pyproject + self._NB_TABLE, tmp_path)
    ...
    def test_pin_round_trips(self, windows_pyproject, tmp_path):
        binaries = tmp_path / "binaries" / "windows"
        binaries.mkdir(parents=True)
        (binaries / "sdk.dll").write_bytes(b"MZ payload")
        lock = _build(windows_pyproject + self._NB_TABLE, tmp_path)
        again = loads(dumps(lock))
        assert again.native_binaries == lock.native_binaries

    def test_serialized_has_native_binaries_table(self, windows_pyproject, tmp_path):
        binaries = tmp_path / "binaries" / "windows"
        binaries.mkdir(parents=True)
        (binaries / "sdk.dll").write_bytes(b"MZ payload")
        text = dumps(_build(windows_pyproject + self._NB_TABLE, tmp_path))
