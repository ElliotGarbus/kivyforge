...
todos:
  ...
  - id: phase3b-macos-signing
    ...
    status: pending
  - id: phase4-linux
    content: "Phase 4 (WSL2 on the Windows machine): Linux backend - overlay, pylock.linux.toml, runtime/wheels, user-selectable artifact via package -f folder/-f appimage (.deb/.rpm/Flatpak external), GL/headless testing (WSLg/software GL/xvfb), doctor checks, examples/linux/. Stop for review."
    status: pending
  - id: phase5-windows
...
- **Scope principle (all platforms):** kivyforge **builds and signs the runnable application artifact**; wrapping it into an installer/container is **out of scope** and delegated to dedicated external tools (the docs recommend options per platform). The seam is clean: kivyforge emits the finished, signed artifact; the external tool ingests it. Signing stays with kivyforge (it must operate on the bundled binaries); only the installer/container step is external. Per-platform runnable artifacts:
  ...
  - Linux: user-selectable runnable artifact via `package -f` (`folder` and `appimage`); `.deb`/`.rpm`/Flatpak external.
...
- **Examples:** a `examples/cross_platform/` area for portable apps plus top-level per-platform dirs (`examples/ios/`, `examples/macos/`, ...) for platform-specific feature showcases, with `examples/wheels/<platform>/` for per-platform wheels.
...
- **Host:** WSL2 Ubuntu on the Windows machine (native x86_64, matches the common Linux distribution target; avoids arm64/emulation from a Mac VM). Docker/Podman (ubuntu/manylinux) as a reproducible/headless complement for artifact builds + CI.
- **Kivy needs OpenGL:** interactive smoke tests via WSLg (D3D12/Mesa GL, with `LIBGL_ALWAYS_SOFTWARE=1` as fallback); automated tests run headless via `xvfb-run` + software GL (or Kivy headless providers).
- `[tool.kivy.linux]` overlay, `pylock.linux.toml`, Linux runtime + wheel resolution, `platforms/linux` backend, doctor checks, `examples/linux/`.
- **User-selectable runnable artifact via `package -f <format>`** (Linux has no universal app-bundle convention, so the format selector earns its keep here, mirroring Briefcase's selectable Linux formats): kivyforge emits `-f folder` (run-from-folder app dir + launcher) and `-f appimage` (self-contained portable). Native packages `.deb`/`.rpm`/Flatpak stay **external** (docs recommend `fpm`/`appimagetool`/`flatpak-builder`). Exact format set/defaults finalized in the Linux spec.
- **Stop / review.**
