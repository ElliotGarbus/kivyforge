...
``kivyforge lock`` reads each declared ``[tool.kivy.ios.native.xcframeworks]``
...
Those pins land in ``pylock.ios.toml`` so ``kivyforge build`` can re-fetch the
    ...
    *,
    ...
    if offline:
        raise XcframeworkResolverError(
            ...
            f"  Re-run `kivyforge lock` with network access, or vendor the archive "
