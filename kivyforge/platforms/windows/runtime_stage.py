...
def stage_runtime(
    ...
    *home* is the final CPython prefix (``<bundle>\\python``). It is emptied and
    ...
    try:
        ...
        extracted = _extract(archive, tmp, runtime.provider, writes_to=(home,))
        if home.exists():
            shutil.rmtree(home)
...
def _refuse_case_collisions(
    ...
    if problem:
        raise WindowsBundleError(
            problem + " Windows filesystems are case-insensitive, so this runtime "
            "archive cannot be staged on a Windows host at all."
        )
