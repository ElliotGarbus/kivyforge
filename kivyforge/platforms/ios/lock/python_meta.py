...
``kivyforge lock`` records the pinned Python.xcframework's URL + SHA-256 and
...
def read_ios_floor(archive_path: str | Path) -> str:
    ...
    slice, which is the device floor that kivyforge validates against.
    ...
    def _resolve(self, url: str, version: str, *, offline: bool) -> tuple[str, str]:
        ...
        if offline:
            raise PythonXcframeworkError(
                ...
                f"({filename}).\n"
                f"  Run `kivyforge lock` online once, or `kivyforge build` to "
