...
1. **Wheel-embedded xcframeworks** — a wheel may ship a ``<pkg>.frameworks/``
   ...
   ``kivyforge build`` walks installed wheels and copies each one out.
2. **Standalone native xcframework archives** — ``[[tool.kivyforge.xcframeworks]]``
...
def read_xcframework_slices(xcframework: Path) -> tuple[str, ...]:
    ...
    returned sorted for a deterministic lockfile. ``kivyforge lock`` records them
    ...
    *,
    ...
    if prior is not None:
        ...
        raise FrameworkConflict(
            ...
            "  Pin a single provider, or remove the duplicate. kivyforge does not "
