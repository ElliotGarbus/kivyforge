...
LEGACY_VERBS: dict[str, str] = {
    "recipes": "No recipes in 3.0. `kivyforge doctor` shows what's pinned in the lock.",
    "create": "Replaced by `init`. Migration: cd <project> && kivyforge init",
    "update": "Replaced by `upgrade`. Run `kivyforge upgrade`.",
    "pip": "Removed. Edit [project].dependencies in pyproject.toml and run `kivyforge lock`.",
    "pip3": "Removed. Edit [project].dependencies in pyproject.toml and run `kivyforge lock`.",
    "distclean": "Removed. `kivyforge clean --cache` covers it.",
...
def legacy_verb_message(verb: str) -> str:
    ...
    return (
        f"'{verb}' is not a verb in kivyforge 3.0.\n"
        f"  {remediation}\n"
        f"  See: {MIGRATION_URL}"
    )
