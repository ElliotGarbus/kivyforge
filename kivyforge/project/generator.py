...
# LastUpgradeCheck is below the running Xcode's version. `kivyforge build`
# detects the installed Xcode and passes its encoded version down (see
# kivyforge/cli/build.py); this constant is only the fallback used when Xcode
...
class XcodeProjectGenerator:
    ...
    def _sync_sources(self, project: XcodeProject) -> None:
        ...
        for source_file in ("main.m", "kivyforge_bootstrap.m"):
